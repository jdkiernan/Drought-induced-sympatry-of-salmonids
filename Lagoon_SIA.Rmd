...
author: "Joseph D. Kiernan and Rosealea M. Bond"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
### Important note on software prerequisites and dependencies
To reproduce this analysis, you must install R, RStudio, and the **JAGS** software. 
JAGS is an external dependency required for the Bayesian modeling in the `SIBER` package.
Because JAGS is an external system-level software, R cannot install it automatically for the user.

* Download JAGS here: [JAGS SourceForge](https://sourceforge.net/projects/mcmc-jags/files/JAGS/4.x/) 

The code was tested and verified using JAGS version 4.3.1 (MacOS) and 4.3.2 (Windows).


```{r Load all libraries, echo=FALSE, message=FALSE, warning=FALSE}

rm(list=ls()) # clear environment

# Fallback path binding for Windows if system environment variable is unmapped
if (.Platform$OS.type == "windows" && Sys.getenv("JAGS_HOME") == "") {
  jags_paths <- c(
    "C:/Program Files/JAGS/JAGS-4.3.2",
    "C:/Program Files/JAGS/JAGS-4.3.1",
    "C:/Program Files/JAGS/JAGS-4.3.0"
  )
  for (path in jags_paths) {
    if (dir.exists(path)) {
...
# Check for JAGS System Dependency
# SIBER requires the standalone JAGS software to run. 
# This bit of code intercepts a system crash and provides instructions if JAGS is missing from the local machine
jags_check <- tryCatch({
...
if (!jags_check) {
  stop(
    "\n\n=======================================================================\n",
    "FATAL ERROR: JAGS (Just Another Gibbs Sampler) is missing.\n\n",
    "The 'SIBER' package requires the standalone JAGS system software to run.\n",
    "1. Download and install JAGS for your OS (Windows/Mac/Linux) here:\n",
    "   [https://sourceforge.net/projects/mcmc-jags/files/JAGS/4.x/](https://sourceforge.net/projects/mcmc-jags/files/JAGS/4.x/)\n",
    "2. After installing JAGS, install the R package by running:\n",
    "   install.packages('rjags')\n",
    "3. Restart RStudio and knit the document again.\n",
    "=======================================================================\n\n"
  )
}

# Data manipulation
# Note: plyr is loaded before tidyverse to prevent masking of dplyr functions
library(plyr)
library(tidyverse) # Automatically loads dplyr, tidyr, ggplot2, readr, and tibble
library(reshape2)

# Statistical and isotope analysis
library(rstatix)
library(coin)
library(SIBER)
library(nicheROVER)
...
This variable ($\delta^{13}C_{corr}$) is used in all subsequent analyses. See methods section of manuscript for additional details.
...
Purpose: to summarize the stable isotope data for each salmonid species (Onki = coho salmon and Onmy = steelhead) by month. These data are presented, in part, in Table 4 of the manuscript.
...
# The following line will add red x's for the ML estimated SEAc, if desired
...
Next we re-run the analysis for to generate the "overall" (all monthly data combined) estimates for each salmonid spp.
...
The next chunk calculates the distribution of overlap based on the posterior distributions of the fitted ellipses. Overlap is expressed as a proportion of the non-overlapping area of the two ellipses so values should range from 0 to 1. These results are exploratory and not presented in the manuscript.
...
The next chunk creates a new data.frame (name = bayes.95.overlap.plotting) that combines the above posterior distributions (of proportional overlap) for data summary and visualization. These results are exploratory and not presented in the manuscript.
...
The next chunk of code extracts summary statistics for each distribution.

### Mean, median and modal overlap values based on the posterior distributions of SEAB
These results are exploratory and not presented in the manuscript.
...
# Create density estimate
  d <- density(x)
# Return the 'x' value that has the highest 'y' value (the peak of their distribution)
...
```{r Global nicheRover parameters}
...
These data, in part, appear in Table S5 of the manuscript.
