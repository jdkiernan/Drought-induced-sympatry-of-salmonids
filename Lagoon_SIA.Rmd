...
write_csv(full_results_df, file.path(sia_dir, "Isotope_Wilcoxon_Results.csv"))
```
...
# Save this table to the SIA_Output directory:
write.csv(metrics.table, file.path(sia_dir, "siber_metrics_summary.csv"), row.names = FALSE)
...
ggsave(file.path(sia_dir, "Figure_2.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, 
       device = function(file, ...) quartz(file = file, type = "pdf", ...))
ggsave(file.path(sia_dir, "Figure_2.tif"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = "tiff", compression = "lzw")
```
...
write.table(SEA.B.modes, 
            file = file.path(sia_dir, "SEAb_Modes.csv"), 
            append = F, sep = ',', row.names = T, col.names = NA)

write.table(SEA.B.credibles, 
            file = file.path(sia_dir, "SEAb_Credible_Intervals.csv"), 
            append = F, sep = ',', row.names = T, col.names = NA)
```
...
write.table(SEA.B.overall.modes, 
            file = file.path(sia_dir, "SEAb_Overall_Modes.csv"), 
            append = F, sep = ',', row.names = T, col.names = NA)

write.table(SEA.B.overall.credibles, 
            file = file.path(sia_dir, "SEAb_Overall_Credible_Intervals.csv"), 
            append = F, sep = ',', row.names = T, col.names = NA)
```
...
write.csv(final_overlap_stats, file.path(sia_dir, "SEAb_Posterior_Overlap_Stats.csv"), row.names = FALSE)
...
ggsave(file.path(sia_dir, "Figure_3.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, 
       device = function(file, ...) quartz(file = file, type = "pdf", ...))
ggsave(file.path(sia_dir, "Figure_3.tif"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = "tiff", compression = "lzw")
```
Figure 3 legend: 
...
write.csv(verification_table, file.path(sia_dir, "QC_Data_Validation.csv"), row.names = FALSE)
...
tryCatch({
  ...
  stop("Failed to read 'lagoon_fish_isotope.csv'. Make sure the file is in the 'SIA_Input_Data' directory.")
})
...
# Calculate and save mean overlap
  ...
  write.table(over.mean, file = file.path(sia_dir, paste0("Rover_Overlap_", period, ".csv")), append = F, sep = ',', row.names = T, col.names = NA)
  
# Calculate and save credible intervals
  over.cred <- apply(over.stat * 100, c(1:2, 4), quantile, prob = c(.025, .975), na.rm = TRUE)
  
  write.table(over.cred, file = file.path(sia_dir, paste0("Rover_Cred_", period, ".csv")), append = F, sep = ',', row.names = T, col.names = NA)
...
# Posterior distribution of niche size
  ...
  write.table(fish.size, file = file.path(sia_dir, paste0("Rover_Niche_Size_", period, ".csv")), append = F, sep = ',', row.names = T, col.names = NA)
...
# Save the standard nicheROVER plots to a PDF for diagnostics
  pdf(file = file.path(sia_dir, paste0(period, "_nicheROVER_base_plots.pdf")), width = 7, height = 6)
...
write.csv(export_table, file.path(sia_dir, "NicheROVER_Summary_Table.csv"), row.names = FALSE)
...
if (length(plot_list) == length(periods)) {
  ...
  # Output
  ggsave(file.path(sia_dir, "Figure_4.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, 
       device = function(file, ...) quartz(file = file, type = "pdf", ...))
  ggsave(file.path(sia_dir, "Figure_4.tif"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = "tiff", compression = "lzw")
