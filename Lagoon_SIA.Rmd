...
2. Nonlinear Normalization Model (Kiljunen et al. 2006) takes that $L$ value and applies the modified parameters ($D = 7.018$ and $I = 0.048$) to the McConnaughey and McRoy (1979) equation to calculate the isotopic shift: 
...
# Create results directory and analysis-specific sub-folders
output_dirs <- c(
  sia_dir,
...
# Note the full data set contains additional fisheries metadata which do not contribute to the analysis
...
Purpose: to summarize the stable isotope data for each salmonid species (Onki = coho salmon and Onmy = steelhead) by month. These summary metrics are presented, in part, in Table 4 of the manuscript.
...
# This chunk uses the full_results_df created above
...
# To generate "Overall" metrics, we create a copy of the data and assigned all records to a single "Overall" community.
...
The previous calculations yielded point estimates (i.e., a single value) of overlap based on the maximum likelihood estimated SEAc. To assess how reliable these data are, we need an associated measure of confidence/variance. This is achieved by calculating the Bayesian standard ellipse area (SEAb) and generating credible intervals.
...
# The following line (currently suppresed) can be used to add red x's for the ML estimated SEAc, if desired
...
Although SIBER has a built-in SEAb plot (Siber.density.plot, created above), we will prepare the SIBER output for ggplot which is far easier to customize and manipulate.
...
SEA.B.3 <- as.data.frame(SEA.B.full) %>% 
  ...
  #E nsure chronological and 'Overall' order for Month factor
...
# Generate the summary HTML table for the document
final_overlap_stats %>%
  kbl(
