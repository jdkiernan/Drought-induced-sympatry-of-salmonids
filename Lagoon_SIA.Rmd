...
# Create a universal output directory to house all results gerated by this Rmd
sia_dir <- "SIA_Output"
...
# The full data set contains additional fisheries metadata which do not contribute to the current analysis
...
# --- 2. Overall (Pooled) Tests & Effect Sizes ---
...
# --- 3. Combine Results & Format for Downstream Code ---
...
# Re-add sia_dir after the environment wipe
sia_dir <- "SIA_Output"
if (!dir.exists(sia_dir)) {
  dir.create(sia_dir)
}
...
# To get "Overall" metrics, we created a copy of the data and assigned all records to a single "Overall" community.
...
This is presented as Figure 2 in the manuscript. 
...
create_siber_biplot <- function(plot_data, plot_code) {
  ...
  ggplot(plot_data, aes(x = iso1, y = iso2, col = group, fill = group)) +
    geom_point(size = 0.8) + # Scaled down point size slightly for smaller canvas
    ...
    theme(
      ...
      plot.title = element_text(family = "serif", face = "plain", size = 10, hjust = 0), # Reduced from 12 to 10
      axis.title = element_blank(), 
      axis.text = element_text(family = "serif", size = 9, color = "black") # Reduced from 12 to 9
    )
}
...
# Reduced global axis font size from 12 to 10
...
ggsave(file.path(sia_dir, "Figure_2.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = cairo_pdf)
...
# Pre-calculate the credible intervals using tidybayes
...
# Build the plot with stacked crossbars
Diet.breadth.plot <- ggplot() +
  # 95% Credible Interval (alpha removed, boxes are solid)
  geom_crossbar(data = ci_95, aes(x = Species, y = value, ymin = .lower, ymax = .upper, fill = Species),
                width = 0.15, color = "black", fatten = 0) +
  # 75% Credible Interval 
  geom_crossbar(data = ci_75, aes(x = Species, y = value, ymin = .lower, ymax = .upper, fill = Species),
                width = 0.25, color = "black", fatten = 0) +
  # 50% Credible Interval 
  geom_crossbar(data = ci_50, aes(x = Species, y = value, ymin = .lower, ymax = .upper, fill = Species),
                width = 0.50, color = "black", fatten = 0) +
  # Add the mean as a black point
  geom_point(data = ci_50, aes(x = Species, y = value), size = 1.2, color = "black") +
  scale_fill_manual(values = c("Onki" = "#EAAF80", "Onmy" = "#80B9D9")) +
  labs(y = expression("Dietary niche breadth " (SEA[b], '\u2030' ^2)), x = "Salmonid species") +
  ...
  theme(
    ...
    axis.title.x = element_text(family = "serif", size = 10, margin = margin(t = 7)), 
...
Diet.breadth.plot <- tag_facet(Diet.breadth.plot, 
          ...
          open = "", close = "",
          fontface = 1,
          # NEW: Drop tag size from 3.2 down to 2.8
          size = 2.8, 
...
ggsave(file.path(sia_dir, "Figure_3.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = quartz)

ggsave(file.path(sia_dir, "Figure_3.tif"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = "tiff", compression = "lzw")
```
```{r Figure 3 version 2, echo=TRUE, message=FALSE, warning=FALSE}
...
create_diet_plot <- function(month_key) {
  ...
  p <- ggplot() +
    ...
    theme(
      ...
      # Force solid axis lines and ticks on every single panel
  ...
  if (month_key != "Jun") {
    p <- p + theme(axis.text.y = element_blank(),
                   plot.margin = margin(t = 5, r = 1, b = 5, l = 1, unit = "pt") # <-- THIS LINE
    )
  }
...
#Legend logic (Jun Only) to match Figure 2 perfectly
  #Legend logic (Jun Only) to match Figure 2 text/margins, with crisp black borders
  if(period == "Jun") {
    p <- p + theme(
...
if (length(plot_list) == length(periods)) {
  ...
  # Output
  ggsave(file.path(sia_dir, "Figure_4.pdf"), width = 6.5, height = 3.97, units = "in", dpi = 800, device = quartz)
