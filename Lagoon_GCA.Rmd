---
title: "Explaining gut content analyses (CGA) conducted on Scott Creek Lagoon fish diet samples"
subtitle: "Drought-induced sympatry of two salmonid species: Feeding ecology and trophic
  niches of potential competitors"
date: "2025-12-01"
output:
  html_document:
    toc: true
...
```{r Load libraries and create directories, echo=FALSE, message =FALSE, warning=FALSE}

rm(list=ls()) #clear environment

# Install and load the necessary packages

#IMPORTANT: The following two lines are suppressed here but are needed to install the 'FSAmisc' package. #See also: https://rdrr.io/github/droglenc/FSAmisc/man/dietOverlap.html for package information.

if (!require('remotes')) install.packages('remotes'); require('remotes')
if (!require('devtools')) install.packages('devtools'); require('devtools')

# Only install FSAmisc if it is not already installed
if (!require('FSAmisc')) remotes::install_github('droglenc/FSAmisc') 
...
if (!dir.exists("Lagoon_GCA_Input_Data")) {
  dir.create("Lagoon_GCA_Input_Data")
}

# Create Results Directory and Analysis Subfolders
output_dirs <- c(
  "Lagoon_GCA_Results",
  "Lagoon_GCA_Results/IRI",
  "Lagoon_GCA_Results/Schoener_Overlap",
  "Lagoon_GCA_Results/IndVal",
  "Lagoon_GCA_Results/Figures",
  "Lagoon_GCA_Results/Tables"
)
...
# Load prey (gut content) data set  #332 obs, 26 vars

Data.Full <- read_csv("Lagoon_GCA_Input_Data/OUT.Consumed.csv")

```

## Monthly summaries of fish processed for gut content analysis (Table S2 in manuscript)

```{r Table S2 Fish summary, message=FALSE, warning=FALSE}
...
final_fish_summary %>%
  kbl(
    caption = "Table S2 (partial). Monthly summaries of the number of stomach samples collected from lagoon-rearing juvenile coho salmon (Onki) and steelhead (Onmy)",
...
The package 'DietR' uses diet data to estimate trophic levels, electivity indices, and composite diet indices. We present the Index of Relative Importance (IRI) which relies on three variables: the number (N), mass (M), and frequency of occurrence (FO) of each diet prey category identified in the stomach contents of each sampled individual. More information about the package can be found [here.](https://cran.r-project.org/web/packages/dietr/vignettes/dietr-vignette.pdf)
...
3. Save IRI output - Supplemental Table *S3* in the manuscript

4. Create Figure 1 in the manuscript - IRI Output by species for each month of the study and overall
...
```{r load IRI required libraries, echo=FALSE, message=FALSE, warning=FALSE}
...
IRI.dat <- read.csv('Lagoon_GCA_Input_Data/IRI_input.csv', sep = ",", header = T) 
IRI.study.dat <- read.csv('Lagoon_GCA_Input_Data/IRI_input_entirestudy.csv', sep = ",", header = T) 
...
write.csv(IRI.monthly.df, "Lagoon_GCA_Results/IRI/IRI_Monthly_Output.csv", row.names = FALSE)
...
# The next line (currently suppressed) will print raw list to screen 
#print(IRI.study.output) 

write.csv(IRI.study.df, "Lagoon_GCA_Results/IRI/IRI_Study_Output.csv", row.names = FALSE)

```

### Extract %IRI calculations and present values in a summary table

These data contribute to Figure 1 and also appear as Table S3 in the manuscript
...
knitr::kable(final_table, 
             caption = "Table S3: Index of relative importance values (IRI, %) by month for each prey category identified in the stomachs of juvenile coho salmon (Onki) and steelhead (Onmy) ",
...
### Create table of "overall" IRI values, sorted from highest to lowest. 
These data are used to check and verify the relative order that the prey categories are presented in Figure 1. They also contribute to Table S3 in the manuscript.

```{r Sorted Overall IRI Table and Export}
# Load the taxa groupings to grab the codes
Prey.Codes <- read.csv("Lagoon_GCA_Input_Data/TaxaGroupings.csv") %>%
  select(PreyCategory, Code) %>%
  distinct()

# Filter for 'Overall', join the codes, sort from highest to lowest, and organize columns
overall_ranked_iri <- combined_data %>%
  filter(Month == "Overall") %>%
  select(Species, Prey, PercentIRI) %>%
  left_join(Prey.Codes, by = c("Prey" = "PreyCategory")) %>%
  select(Species, Prey, Code, PercentIRI) %>% # Places 'Code' right before the IRI values
  group_by(Species) %>%
  arrange(Species, desc(PercentIRI)) %>%
  ungroup()

# Export the cleanly ordered data to a CSV file
write.csv(overall_ranked_iri, "Lagoon_GCA_Results/IRI/Overall_Ranked_IRI_By_Species.csv", row.names = FALSE)

# Format the numbers for the display table with 3 decimal places
overall_ranked_iri %>%
  mutate(PercentIRI = case_when(
    PercentIRI == 0 ~ "---",
    TRUE ~ sprintf("%.3f%%", PercentIRI) 
  )) %>%
  knitr::kable(
    caption = "Overall Study Niche Breadth: Prey categories ranked from highest to lowest Index of Relative Importance (%IRI).",
    col.names = c("Predator Species", "Prey Taxon / Category", "Code", "Overall IRI (%)"),
    align = "lllr" 
  ) %>%
  kable_styling(
...
coho.dat <- read.csv('Lagoon_GCA_Input_Data/DietR_Sankey_Coho.csv', sep = ",", header = T) #77 obs, 9 var.

# Step 2: Bring in taxa groupings codes
DietCode.dat <- read.csv('Lagoon_GCA_Input_Data/TaxaGroupings.csv', sep = ",", header = T) #62 obs, 7 var.
...
# Step 4: Adjust Percentage Monthly values so they add up to the entire study value.
...
entirestudy.dat <- read.csv('Lagoon_GCA_Input_Data/DietR_IRI_Output_entirestudy.csv', sep = ",", header = T) #34 obs, 6 var.
...
# Step 4.6 Join monthly sums to coho data
coho3 <- coho2 %>% 
...
coho.plot <- sankeyNetwork(Links = coho.scaled, Nodes = nodes,
                    ...
                    margin = list(left = 50, right = 0)) 

library(htmlwidgets)
coho.plot <- onRender(coho.plot,
  '
  function(el, x) {
    d3.selectAll(".node text")
      /* NEW FILTER: Targets only the starting nodes (zero incoming links) */
...
Second (right) half of Figure 1 (combined below). Note some of the code is repeated form the Coho plot and has been commented out in this section.
...
sh.dat <- read.csv('Lagoon_GCA_Input_Data/DietR_Sankey_Steelhead.csv', sep = ",", header = T)

# Step 2: Bring in taxa groupings codes
DietCode.dat <- read.csv('Lagoon_GCA_Input_Data/TaxaGroupings.csv', sep = ",", header = T)

DietCode1 <- DietCode.dat  %>% 
  select(PreyCategory, Code) %>%   
  distinct(PreyCategory, Code)     

# Step 3: Set up df with Source and Target information
# REVERSED: Month is now the Source (Left), Code is the Target (Right)
steelhead <- sh.dat %>% 
  left_join(., DietCode1) %>% 
  mutate(Source = Month,
         Target = Code,
...
# Step 4.4 Join codes to steelhead data 
steelhead2 <- steelhead %>% 
  left_join(., sh.entirestudy, by = "Code") %>% 
  select(Species.x, Month, Month2, Code, Group, Source, Target, PercentOccurrence, PercentNumber, PercentMass, PercentIRI,
         Study_PercentOccurrence, Study_PercentNumber, Study_PercentMass, Study_PercentIRI) 

# Step 4.5 Calculate the sum of monthly values
...
# Step 4.6 Join monthly sums to steelhead data
steelhead3 <- steelhead2 %>% 
  left_join(., sum.monthly.sh) 
...
sh.scaled <- steelhead3 %>% 
  mutate(PercentOccurrence.scaled = (PercentOccurrence/PercentOccurrence.total)*Study_PercentOccurrence, 
         PercentNumber.scaled = (PercentNumber/PercentNumber.total)*Study_PercentNumber,
         PercentMass.scaled = (PercentMass/PercentMass.total)*Study_PercentMass,
         PercentIRI.scaled = (PercentIRI/PercentIRI.total)*Study_PercentIRI) 
...
# Step 6: Create source and target indexes
sh.scaled$IDsource <- match(sh.scaled$Source, nodes$name) -1 
sh.scaled$IDtarget <- match(sh.scaled$Target, nodes$name) -1

# Step 7: Dynamically extract the exact group assignments from the link data
# Since Target is the Taxa in this reversed plot, we map the node name to the Target's Group
node_groups <- sh.scaled %>% 
  select(Target, Group) %>% 
  distinct()

nodes <- nodes %>%
  left_join(node_groups, by = c("name" = "Target")) %>%
  # Assign group 11 to months, and use the exact inherited Group for the Taxa
  mutate(group = ifelse(name %in% c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), 
                        "11", as.character(Group))) %>%
  mutate(group = as.factor(group))

# Blank out the month labels so they don't duplicate the Coho labels
nodes$name <- ifelse(nodes$group == "11", " ", as.character(nodes$name))

# Step 8: Unify the Steelhead color scale with the Coho color scale
# (Ensuring domain and range match perfectly)
my_color.sh <- 'd3.scaleOrdinal() .domain(["1", "2", "3", "4", "5", "6", "10", "11"])
                               .range(["#00429D", "#6FA4A6", "#ABBF29", "#A7D3B4", "#6F812A", "#43749E", "gray", "black"])'

# Step 9: Set color for links to correspond with groups
sh.scaled$group <- sh.scaled$Group

# Blank out the month labels so they don't duplicate the Coho labels in the final figure
nodes$name <- ifelse(nodes$group == "11", " ", as.character(nodes$name))
...
sh.plot <- sankeyNetwork(Links = sh.scaled, Nodes = nodes,
                           ...
                           nodeWidth = 5, nodePadding = 15,
                           sinksRight = F, fontSize = 12,
                           margin = list(left = 0, right = 50)) # Push right margin for taxa labels
...
Note: Final figure labeling was done manually outside of the R environment.

#Goal: to make a plot with coho on the left, SH on the right and month in the middle linking the two species plots

```{r Combined Plot, echo=TRUE, message=FALSE, warning=FALSE, cache=TRUE, cache.path="cache/"}

# Format Coho Data (Taxa -> Month)
coho2 <- coho.scaled %>%
  mutate(Target_New = paste0(Month, "_CO")) %>%
  select(Species.x, Source = Code, Target = Target_New, PercentIRI.scaled)

# Format Steelhead Data (Month -> Taxa)
sh2 <- sh.scaled %>%
  mutate(Source_New = paste0(Month, "_SH"),
         Target_New = paste0(Code, "_SH_Taxa")) %>%
  select(Species.x, Source = Source_New, Target = Target_New, PercentIRI.scaled)

# Combine and assign clean color groupings
all <- bind_rows(coho2, sh2) %>%
  mutate(Group = case_when(
    grepl("^AME", Source) | grepl("^AME", Target) ~ "1",
    grepl("^GAM", Source) | grepl("^GAM", Target) ~ "2",
    grepl("^GNO", Source) | grepl("^GNO", Target) ~ "3",
    grepl("^TER", Source) | grepl("^TER", Target) ~ "4",
    grepl("^NEO", Source) | grepl("^NEO", Target) ~ "5",
    grepl("^CHI", Source) | grepl("^CHI", Target) ~ "6",
    grepl("^ORC|^EME|^MEG", Source) | grepl("^ORC|^EME|^MEG", Target) ~ "12",
    TRUE ~ "10"
  ) %>% as.factor())

# Create transparent dummy links to force the center gap between right and left halves
dummy_links <- data.frame(
  Species.x = "Dummy",
  Source = paste0(c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), "_CO"),
  Target = paste0(c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), "_SH"),
  PercentIRI.scaled = 0.0001, 
  Group = as.factor("13") 
)

all <- bind_rows(all, dummy_links)

# Explicitly define Node order to prevent stacking when iterations = 0
coho_taxa <- unique(coho2$Source)
coho_months <- paste0(c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), "_CO")
sh_months <- paste0(c("Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), "_SH")
sh_taxa <- unique(sh2$Target)

nodes <- data.frame(name = c(coho_taxa, coho_months, sh_months, sh_taxa))

# Assign Node color groups
nodes <- nodes %>%
  mutate(group = case_when(
    grepl("_CO$|_SH$", name) ~ "11", # Months
    grepl("^AME", name) ~ "1",
    grepl("^GAM", name) ~ "2",
    grepl("^GNO", name) ~ "3",
    grepl("^TER", name) ~ "4",
    grepl("^NEO", name) ~ "5",
    grepl("^CHI", name) ~ "6",
    grepl("^ORC|^EME|^MEG", name) ~ "12",
    TRUE ~ "10"
  ) %>% as.factor())

# Map the exact d3 Color Scale
my_color.all <- 'd3.scaleOrdinal() .domain(["1", "2", "3", "4", "5", "6", "10", "11", "12", "13"])
                               .range(["#00429D", "#6FA4A6", "#ABBF29", "#A7D3B4", "#6F812A", "#43749E", "gray", "black", "orange", "#00000000"])'

# Calculate indices and clean up display labels
all$IDsource <- match(all$Source, nodes$name) - 1 
all$IDtarget <- match(all$Target, nodes$name) - 1

nodes$name <- gsub("_SH_Taxa", "", nodes$name) # Strip the steelhead suffix
nodes$name <- ifelse(grepl("_CO$|_SH$", nodes$name), " ", nodes$name) # Blank the months
...
# Inject JavaScript for margin labels
library(htmlwidgets)
all.plot <- onRender(all.plot,
  '
  function(el, x) {
    // Target Coho Taxa (Left)
    d3.selectAll(".node text")
      .filter(function(d) { return d.targetLinks.length === 0; }) 
      .attr("x", -10) 
      .attr("text-anchor", "end"); 
      
    // Target Steelhead Taxa (Right)
    d3.selectAll(".node text")
...
# Generate high resolution exports
# Due to the complexity of the plot, all labels will be manually manipulated outside of R
library(webshot2)
library(magick)

saveWidget(all.plot, "Lagoon_GCA_Results/Figures/Figure_1_temp.html", selfcontained = TRUE)

webshot(url = "Lagoon_GCA_Results/Figures/Figure_1_temp.html", file = "Lagoon_GCA_Results/Figures/Figure_1.pdf", delay = 0.5, zoom = 6)

webshot(url = "Lagoon_GCA_Results/Figures/Figure_1_temp.html", file = "Lagoon_GCA_Results/Figures/Figure_1_temp.png", delay = 0.5, zoom = 6)
img <- image_read("Lagoon_GCA_Results/Figures/Figure_1_temp.png")
image_write(img, format = "tiff", path = "Lagoon_GCA_Results/Figures/Figure_1.tif", density = "600x600", compression = "lzw")

unlink(c("Lagoon_GCA_Results/Figures/Figure_1_temp.html", "Lagoon_GCA_Results/Figures/Figure_1_temp.png"))
```
## Schoener's similarity index (D)

As a univariate measure of diet overlap, we calculated Schoener's similarity index (D) for each salmonid species during each sample period using the *FSAmisc* package (Ogle 2022). Index values range from 0 to 1 and values ≥ 0.6 are typically considered substantial evidence of dietary overlap (Wallace Jr. 1981).
...
calculate_monthly_overlap <- function(month_abbr, full_data) {
  ...
  output_dir <- "Lagoon_GCA_Results/Schoener_Overlap"
...
Details for each monthly contrast will be saved to the local directory with the file path ("Lagoon_GCA_Results/Schoener_Overlap")
...
# Note: we can access any individual month's full result:
...
write_csv(overlap_summary_table, "Lagoon_GCA_Results/Schoener_Overlap/monthly_diet_overlap_summary.csv")
```
...
These data contribute, in part, to Table 3 in the manuscript
...
final_overlap_table %>%
  knitr::kable(
    caption = "Table 3 (in part) Schoener's diet overlap (D) between coho salmon and steelhead",
...
# Load data set
Fish.Guts.All.Prey <- read_csv("Lagoon_GCA_Input_Data/OUT.Consumed.csv")
...
These results appear in Table 2 of the manuscript
...
final_indicators_table <- bind_rows(formatted_results_list) %>%
  # Convert Month to a factor so it sorts by time (Jun, Jul...)
...
final_indicators_table %>%
  ...
  knitr::kable(
    caption = "Table 2. Diet indicator values (IndVal) by month, derived from the stomach contents of lagoon-rearing juvenile coho salmon and steelhead",
...
To test whether the dietary composition of individual fish differed between coho salmon and steelhead, we employed one-way analysis of similarities (ANOSIM; Clarke 1993) on the same Bray-Curtis matrix used for NMDS ordinations. ANOSIM generates a test statistic (R) scaled between −1 and 1 to describe group (species) differences based on ranked dissimilarities. Positive R-values indicate greater dietary dissimilarity between groups than within each group, whereas an R-value of 0 indicates little or no dietary separation between groups and negative R-values indicate greater dietary dissimilarity within each group than between them (Clarke and Gorley 2001). The statistical significance of the R-statistic was determined with 999 random permutations.
...
# Read in the main data (expect 332 rows, 26 columns)
Fish.Guts.All.Prey <- read_csv("Lagoon_GCA_Input_Data/OUT.Consumed.csv")

# Read in the groupings/codes
#this reduces clutter in the NMDS ordinations
Prey.Codes <- read_csv("Lagoon_GCA_Input_Data/TaxaGroupings.csv")
...
### Run NMDS analysis loop
Note: This chunk of code is slow to run as it loops through all time periods (months + overall)
...
The results summarized in this chunk are used, in part, to populate Table 3 in the manuscript
...
### Create and assemble NMDS plots

```{r Create Figure S1 plot labels}
...
### Generate multi-panel NMDS Ordination plots (Figure S1)

```{r Create multi-panel Figure S1, fig.width=9, fig.height=5, echo=FALSE}
library(grid) 
library(patchwork)
...
all_plots <- map2(results_list, names(results_list), function(res, name) {
  ...
  # Only add the legend to the "Overall" plot
  if (name == "Overall") {
    p <- p + theme(
...
ggsave("Lagoon_GCA_Results/Figures/Figure_S1_NMDS.pdf", final_plot, width = 9, height = 5, dpi = 600)
ggsave("Lagoon_GCA_Results/Figures/Figure_S1_NMDS.tif", final_plot, width = 9, height = 5, dpi = 600, device = "tiff", compression = "lzw")
```

Figure S1. Ordination plots (NMDS) of the stomach contents of juvenile coho salmon (*Oncorhynchus kisutch*; Onki) and steelhead (*O. mykiss*; Onmy) captured in the Scott Creek lagoon during each month of the study (Jun–Dec, panels a–g, respectively). Panel h shows composited data from all months of the study. Points are individual samples and ellipses represent the 95% confidence interval around the centroid of each fish species in ordination space. Vectors illustrate prey categories with significant (*p*< 0.01) loadings on NMDS axes 1 and 2, and prey category codes are defined in Table 1 of the main document. Each panel is an independent ordination and the unitless axes are arbitrary and unique to each time period.
