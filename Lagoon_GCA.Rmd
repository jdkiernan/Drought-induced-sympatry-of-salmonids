...
3. Save IRI output - Supplemental Table *S3*
...
These data are used to check and verify the relative order prey categories are presented in Figure 1.

```{r Sorted Overall IRI Table and Export}
# Load the taxa groupings to grab the codes
Prey.Codes <- read.csv("TaxaGroupings.csv") %>%
...
write.csv(overall_ranked_iri, "results/Overall_Ranked_IRI_By_Species.csv", row.names = FALSE)
...
sh.dat <- read.csv('DietR_Sankey_Steelhead.csv', sep = ",", header = T)

#Step 2: Bring in taxa groupings codes
DietCode.dat <- read.csv('TaxaGroupings.csv', sep = ",", header = T)
...
saveWidget(all.plot, "Figure_1_temp.html", selfcontained = TRUE)

webshot(url = "Figure_1_temp.html", file = "Figure_1.pdf", delay = 0.5, zoom = 6)

webshot(url = "Figure_1_temp.html", file = "Figure_1_temp.png", delay = 0.5, zoom = 6)
img <- image_read("Figure_1_temp.png")
image_write(img, format = "tiff", path = "Figure_1.tif", density = "600x600", compression = "lzw")

unlink(c("Figure_1_temp.html", "Figure_1_temp.png"))
```
...
ggsave("Figure_S1_NMDS.pdf", final_plot, width = 9, height = 5, dpi = 600)
ggsave("Figure_S1_NMDS.tif", final_plot, width = 9, height = 5, dpi = 600, device = "tiff", compression = "lzw")
```
